...
export default function Checkin() {
      ...
      if (user) {
        ...
        try {
          ...
          if (courses.length === 1) {
            ...
            if (
              userPendingData &&
              userPendingData.section &&
              !courseSections.includes(userPendingData.section)
            ) {
  ...
  useEffect(() => {
    const updateSections = async () => {
      if (selectedCourse) {
        const coursesWithSameName = availableCourses.filter(
          (course) =>
            course.id === selectedCourse ||
            course.courseCode ===
              availableCourses.find((c) => c.id === selectedCourse)?.courseCode
        );
        const courseSections = [
          ...new Set(coursesWithSameName.map((c) => c.section).filter(Boolean)),
        ];
    ...
    if ("geolocation" in navigator) {
      navigator.geolocation.getCurrentPosition(
        ...
        () => {
          ...
          setError(
            "GPS permission denied. Please enable location access to mark attendance."
          );
        }
      );
    } else {
  ...
  const handleCheckin = async () => {
    ...
    try {
      ...
      let matchingCourse = availableCourses.find(
      ...
      if (!selectedSection || availableSections.length === 0) {
        ...
        const courseWithSection = availableCourses.find(
          (course) =>
      ...
      if (studentSnap.empty) {
        setError(
          "You are not enrolled in this course. Please contact your teacher."
        );
        setLoading(false);
        return;
      }
        ...
        {!user && (
          <div className="text-center">
            <p className="text-gray-600 mb-4">
              Please sign in to check your attendance
            </p>
            <button
              onClick={login}
        ...
        {user && userData && userData.role && userData.role !== "student" && (
          <div className="text-center">
            <div className="mb-4 text-yellow-700 bg-yellow-100 p-3 rounded">
              You are signed in as <b>{userData.role}</b>. Please log out and
              sign in as a student to check in.
            </div>
            <button
              onClick={logout}
                ...
                <div>
                  ...
                  {availableCourses.length === 1 && (
                    <p className="text-xs text-gray-500 mt-1">
                      Only one course available (auto-selected)
                    </p>
                  )}
                </div>
              )}
              ...
              {selectedCourse && (
                <div>
                  ...
                  {availableSections.length === 1 && (
                    <p className="text-xs text-gray-500 mt-1">
                      Only one section available (auto-selected)
                    </p>
                  )}
                  {availableSections.length === 0 && (
                    <p className="text-xs text-gray-500 mt-1">
                      Section information will be added when teacher sets up
                      course sections
                    </p>
                  )}
                </div>
